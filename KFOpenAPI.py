...
class KFOpenAPI(QAxWidget):
    ...
    def CommRqData(self, sRQName, sTrCode, sPrevNext, sScreenNo):
        ...
        if errorCode != ErrorCode.OP_ERR_NONE:
            ...
            QTimer.singleShot(10000, self.requestLoop.quit)
    ...
    def GetCommDataToPandas(self, strTrCode, strRecordName, single=False):
        fldList = CommFldList.FLDLIST[strTrCode]
        if single:
            nCount = 1
        else:
            nCount = self.GetRepeatCnt(strTrCode, strRecordName)
            if nCount > 600:
                nCount = 600
        ...
        for row in range(nCount):
            data = []
            for fld in fldList:
                sValue = self.GetCommData(strTrCode, strRecordName, row, fld["key"])
                if fld["type"] == "int":
                    value = int(sValue) if sValue != '' else 0
                elif fld["type"] == "float":
                    value = float(sValue) if sValue != '' else 0.0
                    if fld.__contains__("devide") and fld["devide"] == "Y":
                        value = value / 100
                else:
                    value = sValue
        ...
        df = pd.DataFrame(mdata, columns=[item['key'] for item in fldList])
    ...
    def SendOrder(self, sRQName, sScreenNo, sAccNo, nOrderType, sCode, nQty, sPrice, sStop, sHogaGb, sOrgOrderNo):
        ...
        QTimer.singleShot(3000, self.orderLoop.quit)
        # QTimer.singleShot(3000, self.orderLoop.exit)
    ...
    def GetCommFullData(self, strTrCode, strRecordName, nGubun):
        ...
        return self.dynamicCall("GetCommFullData(QString, QString, long)", strTrCode, strRecordName, nGubun)
    ...
    def ReceiveTrData(self, sScrNo, sRQName, sTrCode, sRecordName, sPrevNext):
        ...
        """
        # print(time.asctime(), "OnReceiveTrData")
...
class ErrorCode(object):
    ...
    CAUSE = {
        ...
        -201 : "주문과부하",
        -202 : "조회입력값(명칭/누락) 오류",
        -203 : "종목코드 오류(거래소 시세과금 미등록 또는 틀린 종목코드)",
        -300 : "주문입력값 오류",
        -301 : "계좌비밀번호를 입력하십시오.",
...
class CommFldList():
    FLDLIST = {
        "opw30001": [
            ...
            {"key": "체결수량", "type": "int"},
            {"key": "미체결수량", "type": "int"},
            {"key": "주문표시가격", "type": "float"},
            {"key": "주문가격", "type": "float"},
            {"key": "조건표시가격","type": "float"},
            {"key": "상태구분", "type": "int"},
            {"key": "통화코드", "type": "str"},
            {"key": "주문시각", "type": "str"},
            {"key": "원주문번호", "type": "int"},
        ...
        "opw30003": [
            ...
            {"key": "평균단가", "type": "float"},
            {"key": "현재가격", "type": "float"},
            {"key": "평가손익", "type": "float", "devide": "Y", "color": "Y"},
            {"key": "약정금액", "type": "float", "devide": "Y"},
            {"key": "평가금액", "type": "float", "devide": "Y"},
            {"key": "수익율", "type": "float", "devide": "Y", "color": "Y"},
            {"key": "수수료", "type": "float", "devide": "Y"},
            {"key": "통화코드", "type": "str"},
        ],
        "opw30007": [
            {"key": "종목코드", "type": "str"},
            ...
            {"key": "수량", "type": "int"},
            {"key": "매도수구분", "type": "int"},
            {"key": "매입표시가격", "type": "float"},
            {"key": "청산가격", "type": "float"},
            {"key": "청산손익", "type": "float", "devide": "Y", "color": "Y"},
            {"key": "수수료", "type": "float", "devide": "Y"},
            {"key": "순손익", "type": "float", "devide": "Y", "color": "Y"},
            {"key": "매입일자", "type": "str"},
            {"key": "청산일자", "type": "str"},
            {"key": "체결시간", "type": "str"},
            {"key": "통화코드", "type": "str"},
        ...
        "opw30009": [
            {"key": "외화예수금", "type": "float", "devide": "Y"},
            {"key": "선물청산손익", "type": "float", "devide": "Y"},
            {"key": "선물평가손익", "type": "float", "devide": "Y"},
            {"key": "예탁자산평가", "type": "float", "devide": "Y"},
            {"key": "포지션증거금", "type": "float", "devide": "Y"},
        ],
        "opc10002": [
            {"key": "현재가", "type": "float"},
            {"key": "거래량", "type": "int"},
